...
\* Set with the existing classes of conflict.
CONSTANT CONFLICT_CLASSES

\* Identifies the conflict class of a given message.
CONSTANT CL(_)
...
GenericBroadcast == INSTANCE GenericBroadcast WITH
    CONFLICTR <- LAMBDA m1, m2: CL(m1) = CL(m2) /\ CONFLICTR(m1, m2)
...
LOCAL ComputeGroupSeqNumberHandler(g, p, msg, ts) ==
    /\ LET
       ...
       IN
        /\ \/ /\ HasConflict(g, p, msg)
              /\ K' = [K EXCEPT ![Class][g][p] = K[Class][g][p] + 1]
              /\ PreviousMsgs' = [PreviousMsgs EXCEPT ![Class][g][p] = {msg}]
           \/ /\ ~HasConflict(g, p, msg)
              /\ PreviousMsgs' = [PreviousMsgs EXCEPT ![Class][g][p] =
...
LOCAL HasNecessaryVotes(g, p, msg, ballot) ==
    ...
    /\ Memory!Contains(g, p, LAMBDA n: msg.id = n[1].id /\ n[2] = "S1")
...
LOCAL InitProtocol ==
    /\ K = [c \in CONFLICT_CLASSES |-> [i \in Groups |-> [p \in Processes |-> 0]]]
    /\ Memory!Init
